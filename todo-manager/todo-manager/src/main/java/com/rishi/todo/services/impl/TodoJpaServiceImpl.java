...
import com.rishi.todo.dao.TodoRepository;
...
public class TodoJpaServiceImpl implements TodoService {
   @Autowired
    ...
    public Todo getTodo(int todoId) throws ParseException {
        return todoRepository.findById(todoId).orElseThrow(()->new ResourceNotFoundException("Todo not found given id", HttpStatus.NOT_FOUND));
    }

    @Override
    public Todo updateTodo(int todoId, Todo todo) {
        Todo todo1 = todoRepository.findById(todoId).orElseThrow(() -> new ResourceNotFoundException("Todo not found given id", HttpStatus.NOT_FOUND));
    ...
    public void deleteTodo(int todoId) {
        Todo todoD = todoRepository.findById(todoId).orElseThrow(() -> new ResourceNotFoundException("Todo not found given id", HttpStatus.NOT_FOUND));
